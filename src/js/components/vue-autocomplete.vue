    ...
    >
      <ul :class="`${getClassName('ulist')}`">
        <li v-for="(data, i) in json" :class="activeClass(i)">
          ...
          >
            <div
              v-if="onShouldRenderChild"
              v-html="onShouldRenderChild(data)"
            ></div>
            <div v-if="!onShouldRenderChild">
              <b class="autocomplete-anchor-text">{{
                deepValue(data, anchor)
              }}</b>
...
export default {
    ...
    methods: {
        ...
        composeParams(val) {
            const encode = (val) =>
                this.encodeParams ? encodeURIComponent(val) : val;
